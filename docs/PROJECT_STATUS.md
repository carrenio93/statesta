...
| Q-NEW-AS | **8 lineup-only players + the 679 S10 thin-seeds all await `/players` bio enrichment.** After S11 `curated.players` holds **687** rows, every one thin (name + maybe photo, all bio NULL). The 8 new ones (unused subs) additionally have **no photo** — any photo-showing UI needs a null-photo fallback. | Completeness of the `players` entity; the player-bio enrichment session (recommended Session 12) | Claude | **Open — new in Session 11.** Direct driver for the `/players` enrichment worker. Not a bug (absent ≠ invented, D-090/D-095); just the enrich half of seed-then-enrich still pending. |

---

## 9. Current Active Task

**Session 11: Sync Workers — EPL-2025 `lineups` (`/fixtures/lineups`)** — ✅ COMPLETE → **third per-fixture worker live; per-fixture event-data set for EPL now complete; Player-Props launch dataset (D-022) complete**

One deliverable, one table. New worker `backend/statesta_sync/lineups.py`: `GET /fixtures/lineups?fixture=<id>` per fixture → land raw → upsert `curated.lineups` on the **three-column** `(fixture_id, team_id, player_id)` key (`uq_lineups`), one row per named player across the starting XI **and** the bench. `formation` + `coach` (`coach_name`/`coach_source_ref`) denormalised onto every row (D-065); `is_starting` from **array membership** (startXI vs substitutes), never `grid`; `grid`/`jersey_number`/`position`/`player_name` per player. Locked rulings: **D-095** (read-mostly player seed — SELECT then `INSERT … DO NOTHING` on `(sport,source,source_ref)`, never DO-UPDATE, so no `photo_url` clobber; `name` NOT NULL → raise), **D-096** (three-column conflict target; `team_id` excluded from update set; `is_starting` updatable), **D-097** (Q-NEW-AO answer for lineups: `source_extra` safety-net + logging on, cosmetics dropped-not-warehoused). Intra-payload dedupe guards the vendor double-list case. Reused: skip-set resume, per-fixture N-or-0 txn, `--limit`/`--sleep`, `/status` + `last_headers`. `--entity lineups` dispatch + `curated.lineups` registered in the admin guardrail (empty set). Commit `715b0ae`.

**Validation (all criteria met):** EPL (league 39) season 2025 → **380/380 fixtures, 15,189 rows** (40×371 + 39×7 + 38×2 — real short benches), **8 players thin-seeded** (unused substitutes, all `player_match_stats`=0, none ever started — the exact case D-095's insert path exists for), **679 pre-existing players untouched, 0 photo clobber**; **exactly 2 team blocks + 22 starters per fixture** on all 380; **grid↔is_starting crosstab clean across all 15,189 rows** (0 starters without grid, 0 subs with grid — the array-membership rule inferred from one probe fixture holds at season scale); **zero orphaned FKs** (fixture/team/player); **`source_extra` NULL throughout** (Q-NEW-AO/D-097 holds — no surprise leaf); both `no_stats`/`failed` buckets empty; **coach non-null on all 760 blocks** (Q-NEW-AH = 0.00% for EPL). **INSERT proven on the full run + DO-UPDATE proven separately** (0-API landed-raw re-upsert: 40/40 updated in place, `created_at` preserved, `updated_at` advanced, ids stable, `team_id` structurally excluded from the update set).

**State of the schema:** Curated Layer = COMPLETE and APPLIED. Raw = minimal landing table live. **Ingestion pattern now proven across five shapes** (static spine, first event-data table, first per-fixture single-table, first per-fixture two-table, and now the three-column-key single-table with a read-mostly seed) plus cross-run resolution, intra-run seed-then-reference, and crash-safe resumability. Ingested for EPL 2025: leagues, league_seasons, venues, teams, standings, fixtures, match_statistics, players (thin), player_match_stats, **lineups**. **The per-fixture event-data set is now complete.** Not yet ingested: **odds** (gated on Q-NEW-AL coverage check); players still need **bio enrichment from `/players`** (D-090/D-095 — now 687 rows, all thin; Q-NEW-AS). Still not designed: full Raw layer, Computed, User, Configuration. **D-084 size: 25,267,347 bytes (24 MB, 6.02%);** `curated.lineups` = 3960 kB; `raw.api_responses` remains the metric that will approach the 400 MB Pro trigger first (grows per-call as endpoints/leagues are added).

*(Prior: 10 — `players` + `player_match_stats` ✅; 9 — `match_statistics` ✅; 8 — fixtures ✅; 7 — spine ingestion ✅; 6 — ingestion design ✅; 5 — Infrastructure ✅; 4B/4A — schema ✅; 3 — Architecture ✅.)*

---

**Next session (12) — Recommended: player bio enrichment via `/players` (Claude Code):**

- **Goal:** backfill the bio columns (`firstname`, `lastname`, `birth_*`, `nationality`, `height`, `weight`) on all **687** thin `curated.players` rows from the paginated `/players?league=39&season=2025&page=N` endpoint. Completes the `players` entity. The 8 S11 lineup-only players additionally need a photo (Q-NEW-AS).
  1. **Standing first step (D-084):** DB-size check, plain verdict vs the **25,267,347-byte (24 MB, 6.02%)** baseline.
  2. New worker *shape* — **paginated** (D-082), not per-fixture: loop pages → land raw → **UPDATE** existing player rows by `(sport, source, source_ref)`. This is the **enrich half** of D-090/D-095's seed-then-enrich ordering; enrichment owns bio, the seed owns identity — the update set must not regress a good `name`/`photo_url`.
  3. Reuse: raw-first landing, idempotent/skip resume, `--limit`/`--sleep`, `/status` + `last_headers`, `source_extra` catch-all + unmapped logging (re-check the census for this endpoint). `curated.players` already registered in the guardrail — confirm the enrichment update set doesn't collide with an admin-owned column.
- **Why next:** unblocked (no coverage question), completes an entity, has a concrete driver (687 thin rows + 8 photoless), and exercises the pagination path at real scale — distinct from every per-fixture worker built so far.
- **Alternatives (product call — pivot at the start-of-session ritual):**
  - **Odds worker (`/odds`)** — last ingestion gap, higher product value (Opportunities engine), but **gated on Q-NEW-AL** (EPL 2025 `cov_odds=false` — verify which league-seasons have odds coverage first) and more complex (three-table append-only, D-060/061/062/063). Resolve coverage before building.
  - **Greek Super League cross-league test (Q-NEW-AQ)** — run an existing worker against the launch-market league to exercise the EPL-only field maps + read the unmapped log; also the place to fix the S11 seed-cache-on-rollback edge (Q-NEW-AR). Low-risk, slots in anytime.
- **Scope caution (Rule 2):** one deliverable. If enrichment, don't drift into odds or the cross-league test.
...
- **ClickUp (done this session, directly via connector):** Session 11 task (`869e3ndwe`) → marked ✅ done. **Session 12 task created** — `869e3nx7r`, status backlog: "Session 12 — Player bio enrichment via /players (backfill the thin-seeded players)". **Do not create a duplicate** — just mark `869e3nx7r` done at Session 12 close.
...
*Last updated: 2026-07-12 (end of Session 11 — EPL-2025 `lineups` ingested & validated via `/fixtures/lineups`: 380/380 fixtures, 15,189 rows, 8 unused-sub thin-seeds, 679 players untouched, grid↔is_starting crosstab clean at scale, zero orphans, zero unmapped; INSERT + DO-UPDATE both proven; the per-fixture event-data set for EPL is now complete; D-095/096/097 logged, Q-NEW-AR/AS opened; DB now 25,267,347 bytes / 24 MB / 6.02% of the Pro trigger; commit `715b0ae`)*
