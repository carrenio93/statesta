...
| Q-NEW-AR | **Seed cache is updated before the fixture commits.** In `lineups._resolve_or_seed_player`, a newly-seeded player's id is written into the in-memory `player_ids` cache *inside* the per-fixture transaction, before commit. If that fixture later **rolls back** (unresolved team → D-092 raise, a parse failure, etc.), the DB insert is undone but the cache still holds the phantom id — and a *later* fixture in the same run referencing that same new player would build a lineup row pointing at an id that was never committed, then fail its FK, cascading one rollback into a second. | The `lineups` (and any future read-mostly-seed) worker's correctness on messier leagues | Claude | **Open — new in Session 11. Cannot fire on EPL** (all teams pre-seeded so D-092 never raises; dedupe prevents the double-affect abort; parse failures near-impossible on completed EPL — so no fixture rolls back). Also self-heals across runs (next run's preload re-reads a clean cache). **Real on a messier league** where seeds *and* rollbacks can co-occur. Fix: stage seeded ids and merge into the persistent cache only *after* the fixture commits. **Sequence the fix with the Greek Super League cross-league work (Q-NEW-AQ).** |
...
**Session 12: Sync Workers — EPL-2025 `player_bio` (`/players`)** — ✅ COMPLETE → **first paginated worker live; `curated.players` bio enrichment done for the EPL-2025 roster; the `/players` endpoint shape (roster, not appearances) is now understood and documented**

One deliverable, one table. New worker `backend/statesta_sync/player_bio.py`: `GET /players?league=39&season=2025&page=N` looping **pages** of the league-season roster → land raw per page → enrich `curated.players` via `upsert_returning_id` on `(sport, source, source_ref)`. **First worker whose unit of work is a PAGE, not a fixture** (so `--limit`/txn/N-or-0 are all page-scoped). `height`/`weight` stored **verbatim as text** (`"193"`/`"76"` — no parse, no unit, per §5.2); `birth_date` ISO-parsed; `name` **excluded from the update set** (seed owns identity, D-090/D-095 — the API's abbreviated `"C. Gakpo"` never overwrites the seeded `"Cody Gakpo"`, proven live); `updated_at` never hand-set (trigger owns it). Locked rulings: **D-098** (enrich-AND-insert — never-played roster members are born fully-enriched), **D-099** (per-player conditional `photo_url` — fill NULLs, never blank a good one; a no-op on EPL, logged as untested insurance), **D-097 precedent** (`age`/`injured`/`statistics` dropped-not-warehoused; unmapped logging on). Page-1-fatal / later-pages-logged asymmetry; league-existence gate (no league FK on `players`). `--entity player_bio` in `LOOP_ENTITIES`. Commit `80de72e`.

**Validation (all criteria met):** EPL (league 39) season 2025 → **34/34 pages, 680 rows enriched** (0 insert + 680 update; Harry Gray inserted during the `--limit 1` smoke → **687 → 688** total); **649/688 players now carry bio**; the **39-row residual is fully characterised** (29 appeared-then-left the EPL roster + 2 stats-only + 8 bench-only — all *off* the 2025 `/players` roster, none a worker failure → Q-NEW-AT); **`name` preserved on all 687** (0 changed), **0 existing photos blanked**, honest-NULL held (Harry Gray's absent bio fields stayed NULL, D-086); **`source_extra` NULL on all 688**, unmapped-leaf log **empty across all 34 pages** (Q-NEW-AO/AQ stay closed for EPL). **INSERT proven + idempotent 0-API replay proven** (680 re-upserted identical, `created_at` preserved, `updated_at` advanced on exactly the 680, the 8 off-roster rows untouched — an independent confirmation the residual is who we think it is). Secret scan clean; committed local, **not pushed**.

**State of the schema:** Curated Layer = COMPLETE and APPLIED. Raw = minimal landing table live. **Ingestion pattern now proven across six shapes** (static spine, first event-data table, first per-fixture single-table, first per-fixture two-table, three-column-key single-table with a read-mostly seed, and now the **first paginated enrich-and-insert worker**) plus cross-run resolution, intra-run seed-then-reference, and crash-safe resumability. Ingested for EPL 2025: leagues, league_seasons, venues, teams, standings, fixtures, match_statistics, **players (now bio-enriched)**, player_match_stats, lineups. **The per-fixture event-data set + the `players` entity are complete for EPL.** Not yet ingested: **odds** (gated on Q-NEW-AL coverage check). Known bounded gap: the 39-row off-roster bio residual (Q-NEW-AT). Still EPL-only-validated: all field maps (Q-NEW-AQ). Still not designed: full Raw layer, Computed, User, Configuration. **D-084 size: 25,701,523 bytes (24.51 MiB, 6.13%);** +434 kB this session (almost all the 35 landed `/players` raw pages); `raw.api_responses` remains the metric that will approach the 400 MB Pro trigger first.

*(Prior: 11 — `lineups` ✅; 10 — `players` + `player_match_stats` ✅; 9 — `match_statistics` ✅; 8 — fixtures ✅; 7 — spine ingestion ✅; 6 — ingestion design ✅; 5 — Infrastructure ✅; 4B/4A — schema ✅; 3 — Architecture ✅.)*

---

**Next session (13) — Recommended: Greek Super League cross-league validation + the Q-NEW-AR phantom-id fix (Claude Code):**

- **Goal:** prove the ingestion foundation on the **launch market** before adding the last data type (odds) or scaling. Every worker is EPL-only-validated (Q-NEW-AQ); run the existing workers against the Greek Super League for one season and **read every unmapped-leaf log**, characterising coverage differences (sparser stats, any new/differently-spelled leaf safely in `source_extra`, whether `/players` `height`/`weight` carry unit suffixes vs EPL's bare numbers).
  1. **Standing first step (D-084):** DB-size check, plain verdict vs the **25,701,523-byte (24.51 MiB, 6.13%)** baseline.
  2. Resolve the Greek Super League's API-Football league id at the ritual; ingest **spine + fixtures first** so the FK chain exists, then event workers + `player_bio`. Live payload probe before trusting any map on the new league.
  3. **Land the one code change this session owns — Q-NEW-AR:** stage seeded player ids in `lineups._resolve_or_seed_player` and merge into the persistent cache only **after** the fixture commits (kills the phantom-id-on-rollback risk). This is the first context where it can actually fire.
- **Why next:** unblocked (odds is gated on Q-NEW-AL); de-risks the whole ingestion layer against the market we launch in; bundles the AR fix at exactly the point it becomes relevant.
- **Alternatives (product call — pivot at the start-of-session ritual):**
  - **Odds worker (`/odds`)** — last ingestion *type*, highest product value (Opportunities engine), but **gated on Q-NEW-AL** (EPL 2025 `cov_odds=false` — verify coverage first) and more complex (three-table append-only, D-060/061/062/063). Resolve coverage before building.
  - **`/players` fill-gaps (Q-NEW-AT)** — reach the 39-row off-roster residual via per-id `GET /players?id=<n>` (~39 calls) as a small worker or a `--fill-gaps` mode. Low-value, low-risk; slots in anytime.
- **Scope caution (Rule 2):** one deliverable. If cross-league validation, land the AR fix but don't drift into odds or the fill-gaps worker.
...
- **ClickUp (done this session, directly via connector):** Session 12 task (`869e3nx7r`) → marked ✅ done. **Session 13 task created** — `869e470g3`, status backlog: "Session 13 — Greek Super League cross-league validation + Q-NEW-AR phantom-id fix". **Do not create a duplicate** — just mark `869e470g3` done at Session 13 close.

**Standing reminder — push state:** as of end-of-S12 the `player_bio` worker + all prior code (`80de72e`, `04c1ff0`, `715b0ae`, `4bef994`) are **pushed to `origin/main`**. This docs commit itself is the only thing that lands after — push it so `origin` fully reflects S12.
...
*Last updated: 2026-07-13 (end of Session 12 — EPL-2025 `player_bio` ingested & validated via `/players`, the first paginated worker: 34/34 pages, 680 rows enriched, 687→688 (Harry Gray, a never-played roster member, D-098), 649/688 with bio, 39-row off-roster residual fully characterised (Q-NEW-AT), name preserved on all 687 + 0 photos blanked, `source_extra` NULL throughout, idempotent 0-API replay proven; D-098/099 logged, Q-NEW-AT opened, Q-NEW-AS resolved; the `players` entity is complete for EPL; DB now 25,701,523 bytes / 24.51 MiB / 6.13% of the Pro trigger; commit `80de72e`, local-only — `main` is 4 commits ahead of origin, unpushed)*
